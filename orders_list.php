...
for($i = 0; $i < count($list); $i++)
{
	...
	{
		?>
		| <a href="orders_list.php?status=<?php echo rawurlencode($list[$i]); ?>"><?php echo htmlspecialchars($list[$i]); ?></a>
		<?php
	}
}
?>
...
if($ordersCount > 0)
{
	...
	<label>Status: <input type="text" name="status" value="<?php echo htmlspecialchars(@$_GET["status"]); ?>" class="text"></label>
	...
	<th nowrap><?php echo pagination_columnHead("Total value", "total"); ?></th>
	</tr>
	<?php
	for($i = 0; $i < count($list); $i++)
	{
